<template>
  <v-overlay :fixed="true" :opacity="0.9" :value="getArticles"
    style="z-index:12"
  >
    <!-- 뒤로가기 버튼 -->
    <v-btn
    ...
    >
      <v-row>
        <v-col
          cols="12"
        >
          <span style="font-size:28px; font-family:'MapoFlowerIsland'; cursor:default;">{{author}}</span>
          <!-- 저장할건가...? 저장하는거 말고 VR을 봤던 걸 보여주는 방식도 생각해 봤음. -->
          <v-icon
            size="34px"
            style="margin-left: 42px"
            @click="addMyList"
            :class="{'hover-event-goto-image-for-data': true, 'check-my-list' : isCheckMyList}"
          >
            mdi-star
          </v-icon>
        </v-col>
